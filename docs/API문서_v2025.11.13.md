# API 문서 v1.0.0 📡
**Girls Band Tabi - RESTful API 가이드**
...
1. [API 개요 및 설계 원칙](#api-개요-및-설계-원칙)
2. [인증 및 인가](#인증-및-인가)
3. [완전한 엔드포인트 참조](#완전한-엔드포인트-참조)
4. [요청/응답 형식](#요청응답-형식)
5. [오류 처리 및 RFC 7807](#오류-처리-및-rfc-7807)
6. [속도 제한 및 페이지네이션](#속도-제한-및-페이지네이션)
7. [필터링 및 검색 기능](#필터링-및-검색-기능)
8. [파일 업로드 및 미디어 처리](#파일-업로드-및-미디어-처리)
9. [OpenAPI/Swagger 통합](#openapiswagger-통합)
10. [테스트 전략 및 도구](#테스트-전략-및-도구)
...
### 아키텍처
Girls Band Tabi API는 다음과 같은 아키텍처 원칙을 바탕으로 **Spring Boot 3.x**로 구축되었습니다:

- **계약 우선 설계(Contract-First Design)**: OpenAPI 3.1 사양이 API 계약을 정의
- **RESTful 아키텍처**: 명확한 리소스 지향 URL을 가진 REST 원칙 준수
- **마이크로서비스 준비**: 명확한 모듈 경계를 가진 Spring Modulith
- **리액티브 지원**: 필요한 경우 논블로킹 작업을 위한 WebFlux
- **도메인 주도 설계**: 명확한 도메인 경계를 가진 기능 우선 조직

### 핵심 설계 원칙

#### 1. 일관성 우선
- 모든 엔드포인트를 위한 표준화된 `ApiResponse<T>` 래퍼
- RFC 7807 Problem Details를 따르는 일관된 오류 형식
- 통일된 명명 규칙 및 URL 패턴
- 예측 가능한 HTTP 상태 코드 사용

#### 2. 개발자 경험
- 예제가 포함된 포괄적인 OpenAPI 문서
- 테스트를 위한 인터랙티브 Swagger UI
- 복구 지침이 있는 명확한 오류 메시지
- 일관된 필드 명명 및 데이터 타입

#### 3. 설계부터 보안
- 리프레시 토큰 로테이션이 있는 JWT 기반 인증
- 역할 기반 접근 제어(RBAC)
- Redis를 통한 속도 제한
- 입력 검증 및 무결성 검사
- 민감한 작업에 대한 감사 로깅
...
### API 버전 관리 전략
- **URI 버전닝**: 모든 엔드포인트에 `/api/v1/` 접두사 사용
- **후방 호환성**: 버전 비호활 알림을 포함한 시맨틱 버전닝
- **마이그레이션 지원**: 버전별 문서 및 마이그레이션 가이드

### 기술 스택
- **프레임워크**: Spring Boot 3.x, Spring Security, Spring Data JPA
- **데이터베이스**: Flyway 마이그레이션을 가진 PostgreSQL
- **캐싱**: Lettuce 커넥터를 가진 Redis
- **저장소**: 미디어 파일을 위한 Cloudflare R2
- **관찰가능성**: Micrometer, Prometheus, Actuator
...
### 개요
API는 다음 기능을 가진 **JWT (JSON Web Token)** 기반 인증을 사용합니다:
- 단기 지속 액세스 토큰(15분)
- 로테이션을 가진 장기 지속 리프레시 토큰(7일)
- 역할 기반 접근 제어(RBAC)
- Redis를 통한 세션 관리
- 선택적 OAuth2 통합
...
#### 사용자 역할
- **USER**: 기본 권한을 가진 일반 사용자
- **ADMIN**: 전체 시스템 접근 권한을 가진 관리자
- **PROJECT_ADMIN**: 프로젝트별 관리자 권한
...
**검증:**
- `username`: 유효한 이메일 주소 (필수)
- `nickname`: 비어있지 않은 문자열 (필수)
- `password`: 복잡성 요구사항을 만족하는 비어있지 않은 문자열 (필수)

**응답:** 201 Created with 토큰 쌍
...
**응답:** 200 OK with 새로운 토큰 쌍

**오류 사례:**
- 401: 잘못되거나 만료된 리프레시 토큰
...
선택적 필터링을 사용하여 프로젝트의 장소 목록을 가져옵니다.

**경로 매개변수:**
- `projectId`: 프로젝트 식별자

**쿼리 매개변수:**
- `unitIds`: 단체 ID로 필터링 (선택사항)
- `includeShared`: 공유 장소 포함 (default: false)
- `page`: 페이지 번호 (0 기반, default: 0)
- `size`: 페이지 크기 (default: 20)
- `sort`: 정렬 기준 (e.g., "name,asc")

**응답:** 200 OK with 페이지네이션된 장소 목록
...
```json
{
  ...
  "data": {
    "id": "place-001",
    "name": "Live House Venue",
    "description": "Popular live music venue featuring indie bands",
    "latitude": 35.6762,
    "longitude": 139.6503,
    "address": "1-1-1 Shibuya, Tokyo 150-0001",
    "tags": ["music", "venue", "indie"],
    "units": [
      {
        "id": "unit-001",
        "name": "Girls Band Cry",
        "description": "Main character band"
      }
    ],
    "photos": [
      {
        "id": "photo-001",
        "url": "https://r2.example.com/photos/venue-exterior.jpg",
        "caption": "Venue exterior view",
        "uploadedBy": "user-001",
    ...
    "guides": [
      {
        ...
        "content": "Take JR Yamanote Line to Shibuya Station...",
        "author": "user-001",
    ...
    "comments": [
      {
        "id": "comment-001",
        "content": "Amazing venue with great acoustics!",
        "author": "user-002",
    ...
    "updatedAt": "2024-11-11T19:00:00Z"
  },
  "metadata": {
...
- `lat`: 위도 (필수)
- `lon`: 경도 (필수)
- `radius`: 미터 단위의 검색 반경 (필수)

**응답:** 200 OK with 근처 장소들

#### POST /api/v1/projects/{projectId}/places
새로운 장소를 생성합니다 (관리자 전용).

**인가:** `ADMIN` 또는 `PROJECT_ADMIN`

**요청 모드:**
```json
{
  "name": "New Venue",
  "description": "A great new music venue",
  "latitude": 35.6762,
  "longitude": 139.6503,
  "address": "Tokyo, Japan",
  "tags": ["music", "venue"],
...
**응답:** 201 Created with 장소 세부 정보

#### PUT /api/v1/projects/{projectId}/places/{placeId}
기존 장소를 업데이트합니다 (관리자 전용).

**인가:** `ADMIN` 또는 `PROJECT_ADMIN`

**응답:** 200 OK with 업데이트된 장소 세부 정보
...
### 표준화된 ApiResponse 래퍼
모든 API 응답은 오류 사례에 대한 RFC 7807 Problem Details를 구현하는 일관된 `ApiResponse<T>` 래퍼 구조를 사용합니다.
...
- **파일 업로드**: 직접 업로드에는 `multipart/form-data`, 사전 서명된 URL에는 `application/json`

### 날짜 및 시간 형식
- **형식**: UTC 타임존을 가진 ISO 8601 (`YYYY-MM-DDTHH:mm:ssZ`)
- **예제**: `2024-11-12T10:00:00Z`
- **타임존**: 모든 타임스탬프는 UTC로 표시
...
### 필드 명명 규칙
- **camelCase** JSON 필드용
- **snake_case** 쿼리 매개변수용 (레거시 지원이 필요한 경우)
- **kebab-case** URL 경로용
- **UPPER_CASE** 상수 및 오류 코드용
...
### 오류 응답 철학
API는 개발자와 자동화된 시스템 모두가 오류를 이해하고 복구할 수 있도록 일관되고 기계 판독 가능한 오류 응답을 제공하기 위해 **RFC 7807 Problem Details for HTTP APIs**를 구현합니다.
...
API는 남용을 방지하고 공정한 사용을 보장하기 위해 Redis를 사용하여 다단계 속도 제한을 구현합니다.
...
### 업로드 아키텍처
API는 서버 부하를 최소화하고 직접 클라이언트-저장소 업로드를 제공하기 위해 보안 사전 서명 URL 방식을 사용하여 **Cloudflare R2**를 미디어 저장소로 사용합니다.
...
### OpenAPI 사양
API는 문서와 API 소비자를 위한 계약 역할을 모두 수행하는 포괄적인 OpenAPI 3.1 사양을 제공합니다.
...
### 테스트 철학
API는 신뢰성, 성능 및 정확성을 보장하기 위해 여러 레이어를 가진 포괄적인 테스트 전략을 따릅니다.
...
이 포괄적인 API 문서는 개발자들이 Girls Band Tabi API와 성공적으로 통합하는 데 필요한 모든 것을 제공합니다. RESTful 설계 원칙, 포괄적인 오류 처리, 견고한 보안, 광범위한 테스트의 조합은 신뢰할 수 있고 개발자 친화적인 경험을 보장합니다.

추가 지원:
- **API 지원**: api-support@girlsbandtabi.com
- **문서**: https://docs.girlsbandtabi.com  
- **상태 페이지**: https://status.girlsbandtabi.com
- **GitHub Issues**: https://github.com/girlsbandtabi/api/issues

API는 개발자 피드백과 사용 패턴을 바탕으로 지속적으로 발전하고 있습니다. 최신 업데이트와 새로운 기능에 대한 내용은 변경로그와 버전 노트를 확인해 주세요.
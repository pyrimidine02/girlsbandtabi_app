...
public class ImageCropperDelegate implements PluginRegistry.ActivityResultListener {
    ...
    public void startCrop(MethodCall call, MethodChannel.Result result) {
        if (pendingResult != null) {
            safeReplyError(
                    result,
                    "crop_in_progress",
                    "Another crop operation is already in progress.",
                    null
            );
            return;
        }
        ...
        try {
            ...
            finishWithError("start_crop_failed", exception.getLocalizedMessage(), exception);
        }
    }
        ...
        if (requestCode == UCrop.REQUEST_CROP) {
            ...
            if (resultCode == RESULT_OK) {
                try {
                    ...
                    finishWithError("crop_error", exception.getLocalizedMessage(), exception);
                }
                return true;
            } else if (resultCode == UCrop.RESULT_ERROR) {
                final Throwable cropError = data != null ? UCrop.getError(data) : null;
                final String errorMessage = cropError != null
                        ? cropError.getLocalizedMessage()
                        : "Image crop failed.";
                finishWithError("crop_error", errorMessage, cropError);
                return true;
            } else {
                finishWithSuccess(null);
                return true;
    ...
    private void finishWithSuccess(String imagePath) {
        final MethodChannel.Result currentResult = takePendingResult();
        if (currentResult == null) return;
        safeReplySuccess(currentResult, imagePath);
    }

    private void finishWithError(String errorCode, String errorMessage, Throwable throwable) {
        final MethodChannel.Result currentResult = takePendingResult();
        if (currentResult == null) return;
        safeReplyError(currentResult, errorCode, errorMessage, throwable);
    }
    ...
    private MethodChannel.Result takePendingResult() {
        final MethodChannel.Result currentResult = pendingResult;
        clearMethodCallAndResult();
        return currentResult;
    }

    private void safeReplySuccess(MethodChannel.Result result, Object payload) {
        try {
            result.success(payload);
        } catch (IllegalStateException exception) {
            Log.w(TAG, "Result reply already submitted (success).", exception);
        }
    }

    private void safeReplyError(
        ...
        try {
            ...
            Log.w(TAG, "Result reply already submitted (error).", exception);
        }
    }
